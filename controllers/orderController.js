...
module.exports.add_Order = async (req, res, next) => {
	...
	form.parse(req,async(err,fields,files) => {
		...
		const final_amount=fields.final_amount;
		const discount_price=fields.discount_price;		
		const total_amount=fields.total_amount;
		const shipping_address=fields.shipping_address;
		const user_contact=fields.user_contact;
		const pin_code=fields.pin_code;
		const full_address=fields.full_address;	
	 	...
	 	if (!product_id) {
				...
				res.json({ status: 'false', message: 'final amount is null.' });
			}
			else if(!discount_price){
				res.json({ status: 'false', message: 'discount price is null.' });
			}
			else if(!total_amount){
				res.json({ status: 'false', message: 'total amount is null.' });
			}
			else if(!shipping_address){
				res.json({ status: 'false', message: 'shipping address is null.' });
			}
			else if(!user_contact){
				res.json({ status: 'false', message: 'user contact is null.' });
			}
			else if(!pin_code){
				res.json({ status: 'false', message: 'pincode is null.' });
			}
			else if(!full_address){
				res.json({ status: 'false', message: 'full address is null.' });
			}
...
module.exports.get_Order_byAdmin=async(req,res,next)=>{
	// var form = new formidable.IncomingForm();
	// form.parse(req,async (err,fields,files) => {
	...
	// 	if(err)throw err
		...
		const order=await Order.findAll({
			...
			// attributes: { exclude: ['quantity','discount_price','final_amount','total_amount','shipping_address','user_contact','pin_code','createdAt','updatedAt','full_address','status'] }

		})
		order.forEach(async function (result,index){
			...
			const username=await User.findAll({
				...
				if(username.length>0){
					username.forEach(async function(element,index){
						console.log("username = ",element.first_name)	
		...
		productName.forEach(async function(element,index){
			
			ProductNames.push(element.product_name)
			console.log("ProductName===1",ProductNames);
			

		})
	
		const result = order.concat({ ProductNames:ProductNames })
		
		username.push(result)
		console.log("username = ",username)
		// res.json({status:"success",message:"Successfully get data cart",result})
