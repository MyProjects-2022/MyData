...
module.exports.add_product = async (req, res, next) => {
	...
	form.parse(req, async (err, fields, files) => {
		...
		const product_discount_percent = fields.product_discount_percent;
		const brand = fields.brand;
		const benefits = fields.benefits;
		const other_product_info = fields.other_product_info;
		const storage_and_usage = fields.storage_and_usage;
		...
		if (result) {
			...
			if (!product_name) {
				...
				res.json({ status: 'false', message: 'Product name is null.' });
			}

			else if (product_amount == null) {

				res.json({ status: 'false', message: 'Product amount is not null.' });
				...
				if (
					...
					res.json({ status: 'false', message: 'Please select category' });
				}

				// if (files.product_image.name &&!files.product_image.name.match(/\.(jpg|jpeg|png|*)$/i))
				// {
				// 	var err = new Error('');
				// 	res.json({status:'false',message:'Please select .jpg or .png file only'});
				// }
				else if (files.product_image.size > 2097152) {
					res.json({ status: 'false', message: 'Please select file size < 2mb' });
						...
						Product.create(
							{
								...
								storage_and_usage: storage_and_usage,
								product_type: product_type,
								product_slug: product_slug,
							},

						).then((created) => {
							if (created.length==undefined) {
								res.json({
									status: 'success',
...
module.exports.get_search_products = (req, res) => {
	...
	const { limit, offset } = getPagination(page, size, search);
	Product.findAndCountAll({ where: {product_name:{[Op.like]:"%"+search+"%"}}, limit, offset })
		.then(data => {
			const response = getPagingData(data, page, limit);
			res.send(response);
	...
	// Sequelize.query(sql,null,{ raw: true, type: Sequelize.query.SELECT}).then(function (results) {
